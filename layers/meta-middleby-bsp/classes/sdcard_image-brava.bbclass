...
#
#    0                         -> 8*1024                                       - reserverd
#    8*1024                    -> 40*1024                                      - aarch64 spl
#    40*1024                   -> 2048*1024                                    - aarch64 u-boot
#    2048*1024                 -> 4096*1024                                    - boot partition A (kernel, device tree, boot.scr)
#    4096*1024                 -> 6144*1024                                    - boot partition B (kernel, device tree, boot.scr)
#    6144*1024                 -> 6144*1024 + ROOTFS_SIZE                      - root partition A
#    6144*1024 + ROOTFS_SIZE   -> 6144*1024 + 2*ROOTFS_SIZE                    - root partition B
#    6144*1024 + 2*ROOTFS_SIZE -> 6144*1024 + 2*ROOTFS_SIZE +  DATA_SPACE      - data partition
#
#

# This image depends on the rootfs image
IMAGE_TYPEDEP_brava-sdimg = "${SDIMG_ROOTFS_TYPE}"
...
# SD card image name
SDIMG = "${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.brava-sdimg"

IMAGE_CMD_brava-sdimg () {
...
IMAGE_CMD_brava-sdimg_append_sun50i () {
